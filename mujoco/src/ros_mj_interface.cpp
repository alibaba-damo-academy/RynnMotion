...
void RosMujocoInterface::initUsscanROS() {
  ...
  EEPoseTrajLimits eeLimits;
  eeLimits.tcpSpeedMax = 0.1;   // 0.1 m/s max speed
  eeLimits.tcpAccMax   = 0.5;   // 0.5 m/s² max acc
  usscanTrajGen_ = std::make_unique<EEPoseTrajGen>(0.001, eeLimits);

  JointTrajLimits jointLimits;
  jointLimits.velMax = Eigen::VectorXd::Constant(7, 1.0);  // 1 rad/s max joint speed
  jointLimits.accMax = Eigen::VectorXd::Constant(7, 2.0);  // 2 rad/s² max joint acc
  usscanJointTrajGen_ = std::make_unique<JointTrajGen<7>>(0.001, jointLimits);
