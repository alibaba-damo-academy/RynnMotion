...
def create_cubic_spline_with_boundary_conditions(x_values, y_values, config_path):
    ...
    try:
        ...
        boundary_condition = "natural"
        logging.debug(f"Warning: Config file {config_path} not found, using defaults")
...
if __name__ == "__main__":
    ...
    action_chunk = np.array(
        [
            ...
            [3, 3.3, 3, 3, 3.1, 3.6],
            [6, 6.1, 6, 6, 6.2, 6.5],
            [10, 10.1, 10, 10, 10.2, 10.5],
        ]
    )
    shape = action_chunk.shape
    chunk_size = shape[0]
    ndof = shape[1]
    print(action_chunk)
    # interpolator.prepare_trajectory(
    #     joint_positions,
    #     6,
    #     3,
    # )
    interp_action_chunk = interpolator.offline_trajgen(
        action_chunk.flatten(), ndof, chunk_size, 30, 100
    )
    print(np.array(interp_action_chunk).reshape(-1, ndof))
    # for i in range(4):
    #     print(interpolator.update(i))
