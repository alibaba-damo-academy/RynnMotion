...
from RynnLeRobot.hardware.robots.config import RobotConfig

from .robot import Robot


def make_robot_from_config(config: RobotConfig) -> Robot:
    """Create robot instance from configuration. Only supports implemented robots."""
    if config.type == "so101_follower":
        ...
        raise ValueError(f"Unsupported robot type: {config.type}. Available types: 'so101_follower', 'lekiwi'")
