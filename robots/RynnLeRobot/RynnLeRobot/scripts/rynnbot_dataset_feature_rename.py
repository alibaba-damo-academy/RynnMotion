...
def is_episode_file(rel_path, file_name):
    """
    判断文件是否为 episode_数字/*.parquet 或 episode_数字/*.json 格式的文件
    """
    if len(rel_path.parts) >= 2:
        # 检查父目录是否是 episode_数字 格式
        parent_dir = rel_path.parts[-2]  # 倒数第二个部分应该是episode_数字

        # 检查文件扩展名是否是 .parquet 或 .json
        valid_extensions = [".parquet", ".json"]
        file_ext = Path(file_name).suffix.lower()

        if (
            parent_dir.startswith("episode_")
            and parent_dir[8:].isdigit()  # 检查episode_后面的部分是否为数字
            and file_ext in valid_extensions
        ):
            return True
...
def copy_and_replace_keys(input_dir, output_dir):
    ...
    for root, dirs, files in os.walk(input_path):
        ...
        for file_name in files:
            ...
            # 检查是否是需要特殊处理的文件
            if is_episode_file(rel_path / file_name, file_name):
