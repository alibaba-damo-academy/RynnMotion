...
def load_config(config_path: str = "configs/so101.yaml"):
...
def select_device(config: dict) -> str:
    """Prompt user to select follower, leader, or second leader device.
    ...
    if teleop2_available:
        print(t("show_angles_option_second_leader"))

    choice = input(t("show_angles_select_device")).strip()

    if choice == "2":
        return "leader"
    elif choice == "3" and teleop2_available:
...
def main():
    ...
    if device == "follower":
        ...
        device_name = t("dev_type_second_leader")
    ...
    try:
        ...
        if device == "follower":
            ...
            interface = create_robot_interface(
                ...
                mode="teleop",
                config_path=config_path,
                port=teleop_config.get("port_2"),
                calibration_dir=teleop_config.get("calibration_dir_2"),
            )
