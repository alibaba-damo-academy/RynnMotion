...
def process_inverse_meta_episodes_stats(meta_input_path, meta_output_path):
    """
    处理 meta/episodes_stats.jsonl 文件，对其中的统计信息进行逆向转换
    """
    if not meta_input_path.exists():
        logging.warning(f"Meta episodes stats file does not exist: {meta_input_path}")
        return

    logging.info(f"Processing inverse meta episodes stats: {meta_input_path}")
                        ...
                        for key in transform_keys:
                            if key in stats_dict and isinstance(stats_dict[key], dict):
                                ...
                                if any(
                                    ...
                                    # logging.info(
                                    #     f"Inverse transforming {key} stats in line {line_num}"
                                    # )
                                    json_obj["stats"][key] = (
                                        inverse_transform_stats_values(stat_values)
                                    )
...
def copy_other_directories_inverse(input_dir, output_dir):
    """
    复制input_directory中除了data/chunk-*之外的其他目录到output_directory（逆向转换版本）
    """
                ...
                for sub_item in item.iterdir():
                    ...
                    if sub_item.is_file():
                        ...
                        if sub_item.name == "episodes_stats.jsonl":
                            ...
                            process_inverse_meta_episodes_stats(sub_item, dest_sub_path)
...
def process_inverse_directory(input_dir, output_dir):
    """
    逆向处理输入目录中的所有parquet文件
    """
    input_path = Path(input_dir)
    output_path = Path(output_dir)
    ...
    if not chunk_dirs:
        ...
        for chunk_dir in chunk_dirs:
            logging.info(f"Processing inverse chunk directory: {chunk_dir}")
            ...
            for parquet_file in parquet_files:
                ...
                try:
                    process_inverse_parquet_file(parquet_file, output_file)
                except Exception as e:
                    logging.error(f"Error processing inverse {parquet_file}: {str(e)}")

    # 复制其他目录到输出目录
    copy_other_directories_inverse(input_dir, output_dir)
...
if __name__ == "__main__":
    ...
    logging.info(
        f"Inverse converting data from {input_directory} to {output_directory}"
    )
    process_inverse_directory(input_directory, output_directory)
