...
Follows the sequential prompt pattern: follower -> leader -> second leader.
...
def calibrate_second_leader(config: dict) -> str:
    ...
    print(f"\n{'='*60}")
    print(t("calib_header_second_leader"))
    print(f"{'='*60}")
    ...
    try:
        ...
        logging.error(f"Failed to create second teleoperator: {e}")
        return t("calib_status_failed")
