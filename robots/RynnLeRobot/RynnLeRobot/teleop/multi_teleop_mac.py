"""Multi-robot teleoperation for macOS (30 FPS).

Usage:
    $ multi-teleop-mac --dual --master_number 24
"""
import argparse
import mujoco
import glfw
...
class UnifiedMujocoGLFW:
    """Multi-robot teleoperation with MuJoCo GLFW viewer (macOS optimized)."""
    ...
    def __init__(self, is_dual_arm: bool, master_number: int, slave_numbers: list):
        ...
        self.render_freq = 30.0  # Reduced from 60
        ...
        # Defer GLFW/rendering init - must happen on main thread (macOS requirement)
        # These will be called in run() which executes on the main thread
        self._rendering_initialized = False

        # Initialize non-GUI components here (these don't need main thread)
        self.init_scene()  # Load MuJoCo model - doesn't need main thread
    ...
    def init_glfw(self):
        """Initialize GLFW window and OpenGL context (macOS optimized)"""
        try:
            if not glfw.init():
            ...
            glfw.swap_interval(0)  # Disable vsync for manual frame control
    ...
    def get_master_info(self):
        """Load master robot configuration"""
        try:
    ...
    def get_slaves_info(self):
        """Initialize slave robot information"""
        ...
        with self.simulation_mutex:
            ...
            for i, cam_id in enumerate(self.cameras):
                try:
                    ...
                    print(f"Error rendering camera {i}: {e}")
                    continue
    ...
    def _init_rendering_on_main_thread(self):
        """Initialize rendering components - MUST be called on main thread (macOS)"""
        if self._rendering_initialized:
            return

        print("Initializing rendering on main thread...")
        self.init_glfw()  # GLFW window creation - requires main thread on macOS
        self.init_mujoco_rendering()  # MuJoCo rendering context - requires OpenGL context
        self.init_teleop()  # Initialize hardware interfaces
        self._rendering_initialized = True
        print("Rendering initialization complete")

    def run(self):
        """Main run function"""
        try:
            # Initialize GLFW/rendering here on the main thread (macOS requirement)
            self._init_rendering_on_main_thread()
