"""LeKiwi chassis velocity to wheel velocity mapping."""

import numpy as np


def chassis_ikvelocity(chassis_vel):
    """Compute wheel velocities from chassis velocity [vx, vy, omega]."""
    ...
    qdot = np.where(np.abs(qdot) < DEADBAND_THRESHOLD, 0.0, qdot)
