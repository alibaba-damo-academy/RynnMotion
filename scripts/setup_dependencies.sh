...
install_mujoco_linux() {
    ...
    for version in "3.5.0" "3.4.0" "3.3.5" "3.3.4" "3.3.3" "3.3.2" "3.3.1" "3.3.0"; do
    ...
    if [ -n "$MUJOCO_PERMANENT_DIR" ]; then
        ...
        echo ""
        if [ "$MUJOCO_VERSION" != "3.5.0" ]; then
            echo "Note: You have MuJoCo $MUJOCO_VERSION, but the recommended version is 3.5.0."
            read -p "Would you like to install MuJoCo 3.5.0? (y/n) " -n 1 -r
            echo
            if [[ ! $REPLY =~ ^[Yy]$ ]]; then
                echo "Continuing with existing MuJoCo $MUJOCO_VERSION installation."
                return 0
            fi
            echo "Proceeding to install MuJoCo 3.5.0..."
        else
            echo "You already have the recommended version (3.5.0)."
            return 0
        fi
    fi

    echo "MuJoCo 3.5.0 needs to be installed."
    echo ""
    ...
    if [[ $REPLY == "1" ]]; then
        ...
        DOWNLOAD_URL="https://github.com/google-deepmind/mujoco/releases/download/3.5.0/mujoco-3.5.0-${MUJOCO_ARCH}.tar.gz"
        DOWNLOAD_FILE="/tmp/mujoco-3.5.0-${MUJOCO_ARCH}.tar.gz"
        EXTRACT_DIR="/tmp/mujoco-3.5.0-extract"
        ...
        if wget -O "$DOWNLOAD_FILE" "$DOWNLOAD_URL" || curl -L -o "$DOWNLOAD_FILE" "$DOWNLOAD_URL"; then
            ...
            if tar -xzf "$DOWNLOAD_FILE" -C "$EXTRACT_DIR" --strip-components=1; then
                ...
                echo "Installing MuJoCo 3.5.0 to /usr/local/mujoco-3.5.0..."
                mkdir -p /usr/local/mujoco-3.5.0
                cp -r "$EXTRACT_DIR"/* /usr/local/mujoco-3.5.0/

                if [ -f "/usr/local/mujoco-3.5.0/lib/libmujoco.so.3.5.0" ] && [ -d "/usr/local/mujoco-3.5.0/include" ]; then
                    echo "✓ MuJoCo 3.5.0 installed successfully!"
                    echo "  Library: /usr/local/mujoco-3.5.0/lib/"
                    echo "  Headers: /usr/local/mujoco-3.5.0/include/"
                    ...
                    if [[ $REPLY =~ ^[Yy]$ ]]; then
                        ...
                        if ! grep -q "LD_LIBRARY_PATH.*mujoco-3.5.0" ~/.bashrc 2>/dev/null; then
                            echo 'export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/mujoco-3.5.0/lib' >> ~/.bashrc
                        ...
                        echo "   echo 'export LD_LIBRARY_PATH=\$LD_LIBRARY_PATH:/usr/local/mujoco-3.5.0/lib' >> ~/.bashrc"
                        echo "   source ~/.bashrc"
                    fi
                else
            ...
            echo "Extract to /usr/local/mujoco-3.5.0"
            return 1
        fi
    elif [[ $REPLY == "2" ]]; then
        echo "Please follow these manual steps:"
        echo "1. Download MuJoCo 3.5.0 from https://github.com/google-deepmind/mujoco/releases"
        echo "   Direct link: https://github.com/google-deepmind/mujoco/releases/download/3.5.0/mujoco-3.5.0-linux-x86_64.tar.gz"
        echo "2. Extract: tar -xzf mujoco-3.5.0-linux-x86_64.tar.gz"
        echo "3. Move: sudo mv mujoco-3.5.0 /usr/local/mujoco-3.5.0"
...
install_mujoco_macos() {
    ...
    if [ ! -d "$DMG_MOUNT_PATH" ]; then
        ...
        if [[ $REPLY == "1" ]]; then
            echo "Downloading MuJoCo 3.5.0..."
            DOWNLOAD_URL="https://github.com/google-deepmind/mujoco/releases/download/3.5.0/mujoco-3.5.0-macos-universal2.dmg"
            DOWNLOAD_FILE="/tmp/mujoco-3.5.0-macos-universal2.dmg"
            ...
            echo "1. Download MuJoCo 3.5.0 .dmg from https://github.com/google-deepmind/mujoco/releases"
            echo "   Direct link: https://github.com/google-deepmind/mujoco/releases/download/3.5.0/mujoco-3.5.0-macos-universal2.dmg"
    ...
    if [ -d "$DMG_MOUNT_PATH/mujoco.framework" ]; then
        ...
        if [ -f "$MUJOCO_PERMANENT_DIR/lib/libmujoco.${MUJOCO_VERSION}.dylib" ] && [ -d "$MUJOCO_PERMANENT_DIR/include" ]; then
            ...
            if [ -f "/tmp/mujoco-3.5.0-macos-universal2.dmg" ]; then
                rm -f "/tmp/mujoco-3.5.0-macos-universal2.dmg"
