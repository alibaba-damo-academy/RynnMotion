...
from RynnMotion.RynnDatasets.constants import HF_LEROBOT_HOME
from RynnMotion.RynnDatasets.compute_stats import aggregate_stats, compute_episode_stats
from RynnMotion.RynnDatasets.image_writer import AsyncImageWriter, write_image
...
from RynnMotion.RynnDatasets.video_utils import (
...
class RynnDatasetMetadata:
    ...
    def __del__(self):
        """
        Trust the user to call .finalize() but as an added safety check call the parquet writer to stop
    ...
    def create(
        ...
        """Creates metadata for a RynnDataset."""
        obj = cls.__new__(cls)
        obj.repo_id = repo_id
...
class RynnDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        repo_id: str,
        root: str | Path | None = None,
        ...
        1. Your dataset already exists:
            ...
            - On the Hugging Face Hub at the address https://huggingface.co/datasets/{repo_id} and not on
              ...
              prompted to convert it using our conversion script from v2.1 to v3.0.


        2. Your dataset doesn't already exists (either on local disk or on the Hub): you can create an empty
           RynnDataset with the 'create' classmethod. This can be used for recording a dataset or port an
           existing dataset to the RynnDataset format.


        In terms of files, RynnDataset encapsulates 3 main things:
            - metadata:
        ...
        A typical RynnDataset looks like this from its root path:
        .
        ├── data
        │   ├── chunk-000
        │   │   ├── file-000.parquet
        ...
        Args:
            ...
            download_videos (bool, optional): Flag to download the videos. Note that when set to True but the
                ...
                True.
            video_backend (str | None, optional): Video backend to use for decoding videos. Defaults to
                torchcodec when available on the platform; otherwise, defaults to 'pyav'.
            ...
            encoder_threads (int | None, optional): Number of threads per encoder instance. None lets the
                codec auto-detect (default). Lower values reduce CPU usage per encoder.
        """
        super().__init__()
        self.repo_id = repo_id
    ...
    def num_frames(self) -> int:
        ...
        Note: When a subset of the full dataset is requested, we must return the
    ...
    def finalize(self):
        """
        Close the parquet writers. This function needs to be called after data collection/conversion,
        else footer metadata won't be written to the parquet files. The dataset won't be valid and
        can't be loaded as ds = RynnDataset(repo_id=repo, root=HF_LEROBOT_HOME.joinpath(repo))
        """
        self._close_writer()
        ...
        for key in frame:
            ...
            if self.features[key]["dtype"] == "video" and self._streaming_encoder is not None:
                ...
                if frame_index == 0:
                    ...
                    logger.debug(f"Image dir created: {img_path.parent}")
    ...
    def save_episode(
        ...
        if use_streaming:
            ...
            if parallel_encoding and num_cameras > 1 and sys.platform != "darwin":
        ...
        # `meta.save_episode` needs to be executed after encoding the videos
    ...
    def _batch_save_episode_video(self, start_episode: int, end_episode: int | None = None) -> None:
        ...
        Args:
            ...
            end_episode: Ending episode index (exclusive). If None, encodes all episodes from
                start_episode to the current episode.
        """
    ...
    def stop_image_writer(self) -> None:
        ...
        remove the image_writer in order for the RynnDataset object to be pickleable and parallelized.
        """
        if self.image_writer is not None:
    ...
    def _wait_image_writer(self) -> None:
        ...
        if self.image_writer is not None:
            ...
            if self.episode_buffer is not None:
                for key in self.meta.video_keys:
                    paths = self.episode_buffer.get(key, [])
                    if paths and paths[0] is not None:
                        first_path = Path(paths[0])
                        if not first_path.exists():
                            logger.error(
                                f"Image writer completed but {first_path} does not exist. "
                                f"Image writer threads may have crashed silently."
                            )
    ...
    def encode_episode_videos(self, episode_index: int) -> None:
        """
        Use ffmpeg to convert frames stored as png into mp4 videos (fallback for non-streaming encoding).
    ...
    def batch_encode_videos(self, start_episode: int = 0, end_episode: int | None = None) -> None:
        """
        Batch encode videos for multiple episodes (fallback for non-streaming encoding).
        ...
        logger.info(f"Starting batch video encoding for episodes {start_episode} to {end_episode - 1}")

        for ep_idx in range(start_episode, end_episode):
            logger.info(f"Encoding videos for episode {ep_idx}")
            self.encode_episode_videos(ep_idx)

        logger.info("Batch video encoding completed")
    ...
    def create(
        ...
        """Create a RynnDataset from scratch in order to record data."""
        vcodec = resolve_vcodec(vcodec)
        obj = cls.__new__(cls)
        obj.meta = RynnDatasetMetadata.create(
            repo_id=repo_id,
            fps=fps,
