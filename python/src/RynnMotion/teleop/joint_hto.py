#!/usr/bin/env python3
"""
Heterogeneous Teleoperation (Joint Mapping) - single process, no LCM
Master: MuJoCo passive viewer (drag control)
Slave: GLFW rendering with direct joint-to-joint mapping

This implementation uses joint-level teleoperation where master joint positions
are directly mapped to slave joint positions with appropriate transformations.
For EE-based teleoperation with OSC + Differential IK, see ee_hto.py
"""
...
class JointHTOApp:
    def __init__(
        ...
        Args:
            ...
            arm_side: Arm side for mapping - "left" or "right" (default: "left")
        ...
        # Components (initialized later) - only PinKine for joint mapping
        ...
        if self.verbose:
            print("=" * 60)
            print("HETEROGENEOUS TELEOPERATION (Joint Mapping)")
            print("=" * 60)
            print(f"Master robot: {self.master_robot_manager.getRobotName()} (#{master_robot_number})")
            print(f"Slave robot:  {self.slave_robot_manager.getRobotName()} (#{slave_robot_number})")
            print("Master: MuJoCo viewer (drag control)")
            print("Slave:  GLFW rendering (joint mapping control)")
            print("=" * 60)
    ...
    def init_glfw(self):
        ...
        self.window = glfw.create_window(
            window_width,
            window_height,
            "Heterogeneous Teleop (Joint Mapping) - Slave Robot",
            None,
            None,
        )
        if not self.window:
    ...
    def init_master_viewer(self):
        ...
        self.master_viewer = mujoco.viewer.launch_passive(self.master_model, self.master_data, show_left_ui=False, show_right_ui=True)

        # Compute master gripper range
        self.master_gripper_range = self.master_model.actuator_ctrlrange[-1][1] - self.master_model.actuator_ctrlrange[-1][0]
    ...
    def init_teleop(self):
        """Initialize PinKine for joint mapping (minimal setup)"""
    ...
    def _qDirection_to_dirMat(self, qDirection: np.ndarray, m_dof: int) -> np.ndarray:
        """
        Convert qDirection vector to dirMat matrix.

        Args:
            qDirection: Direction vector indexed by slave joints (length = slave DOF)
            m_dof: Master DOF

        Returns:
            Direction matrix (m_dof × s_dof)
        """
    ...
    def _jointMapping_so101piper(self, master_qCmd: np.ndarray) -> np.ndarray:
        """SO101 → Piper joint mapping (same for left and right)."""
        master_qStand = self.master_robot_manager.get_qStandby(self.arm_side)
        ...
        qDirection = np.array([-1, 1, 1, 0, 1, -1])
        dirMat = self._qDirection_to_dirMat(qDirection, m_dof)
    ...
    def _jointMapping_so101fr3(self, master_qCmd: np.ndarray) -> np.ndarray:
        """SO101 → FR3 joint mapping (same for left and right)."""
        master_qStand = self.master_robot_manager.get_qStandby(self.arm_side)
        slave_qStand = self.slave_robot_manager.get_qStandby(self.arm_side)
        ...
        qDirection = np.array([-1, 1, 0, -1, 0, -1, -1])
        dirMat = self._qDirection_to_dirMat(qDirection, m_dof)
    ...
    def _jointMapping_so101ur5e(self, master_qCmd: np.ndarray) -> np.ndarray:
        """SO101 → UR5e joint mapping (different for left and right)."""
        master_qStand = self.master_robot_manager.get_qStandby(self.arm_side)
        slave_qStand = self.slave_robot_manager.get_qStandby(self.arm_side)
        ...
        if self.arm_side == "left":
            qDirection = np.array([-1, -1, -1, -1, 0, -1])
        else:
            qDirection = np.array([-1, 1, 1, 1, 0, -1])
    ...
    def _jointMapping_so101rm75(self, master_qCmd: np.ndarray) -> np.ndarray:
        """SO101 → RM75 joint mapping (different for left and right)."""
        master_qStand = self.master_robot_manager.get_qStandby(self.arm_side)
        slave_qStand = self.slave_robot_manager.get_qStandby(self.arm_side)
    ...
    def run_slave_jointTeleop(self):
        ...
        if self.master_robot_number == 24 and self.slave_robot_number == 22:
            slave_joint_command[:s_dof] = self._jointMapping_so101piper(master_motion_joints)
        elif self.master_robot_number == 24 and self.slave_robot_number == 20:
            slave_joint_command[:s_dof] = self._jointMapping_so101fr3(master_motion_joints)
        elif self.master_robot_number == 24 and self.slave_robot_number == 21:
            slave_joint_command[:s_dof] = self._jointMapping_so101ur5e(master_motion_joints)
        elif self.master_robot_number == 24 and self.slave_robot_number == 23:
            slave_joint_command[:s_dof] = self._jointMapping_so101rm75(master_motion_joints)
        else:
            if self.verbose:
                print(f"⚠️  Unsupported robot combination: master #{self.master_robot_number} → slave #{self.slave_robot_number}")
            slave_joint_command[: min(m_dof, s_dof)] = master_motion_joints[: min(m_dof, s_dof)]

        slave_gripper_idx = self.nctrl - 1
        slave_gripper_ctrlrange_min = self.model.actuator_ctrlrange[slave_gripper_idx][0]
        slave_gripper_ctrlrange_max = self.model.actuator_ctrlrange[slave_gripper_idx][1]
        slave_gripper_cmd = slave_gripper_ctrlrange_min + slave_gripper_ctrlrange_max * master_gripper_cmd / self.master_gripper_range
        ...
        while not self.exit_request.is_set():
            ...
            if self.pyfsm.is_Initr():
                ...
                if time.time() - last_control_time >= self.ctrl_dt:
                    ...
                    # Run joint-to-joint teleoperation
                    self.run_slave_jointTeleop()
    ...
    def run(self):
        ...
        try:
            ...
            sim_thread = threading.Thread(target=self.simulation_thread_func, daemon=True)
            sim_thread.start()
...
def main():
    parser = argparse.ArgumentParser(
        description="Heterogeneous Teleoperation (Joint Mapping)",
        epilog="Supported combinations: SO101 (#24) → Piper (#22), FR3 (#20), UR5e (#21), RM75 (#23)",
    )
    parser.add_argument(
        "--master_number",
        "-mn",
        type=int,
        default=24,
        help="Master robot ID (default: 24 for SO101)",
    )
    parser.add_argument(
        "--slave_number",
        "-sn",
        type=int,
        default=22,
        help="Slave robot ID (default: 22 for Piper, options: 20=FR3, 21=UR5e, 23=RM75)",
    )
    ...
    parser.add_argument(
        ...
        choices=["left", "right"],
        default="left",
        help="Arm side for mapping: left (default) or right",
    )
